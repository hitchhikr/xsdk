...
/*------------------------------------------------------------------------------------
 * setfillcol -- Set fill column to n.
 *------------------------------------------------------------------------------------
 */
setfillcol(f, n)
{
...
/*----------------------------------------------------------------------------------
 * getccol -- Return current column.  Stop at first non-blank given TRUE argument.
 *----------------------------------------------------------------------------------
 */
getccol(bflg)
int bflg;
...
/*------------------------------------------------------------------------------------
 * insbrace -- insert a brace into the text here...we are in CMODE 
 *------------------------------------------------------------------------------------
 */
insbrace(n, c)  
int n;                          /* repeat count             */
int c;                          /* brace to insert (always { for now)   */
{
    ...
    target -= 1;
    target -= target % (tabsize == 0 ? (tabmask + 1) : tabsize);
...
#ifdef KANJI
        return (ldelete(nld, FALSE, FALSE));
#else
...
indent(f, n)
{
    ...
    while (n--)
    {
        ...
        if (lnewline() == FALSE
            || ((i = nicol / (tabmask + 1)) != 0 && linsert(i, '\t') == FALSE)
            || ((i = nicol % (tabmask + 1)) != 0 && linsert(i,  ' ') == FALSE))
        {
            return (FALSE);
        }
...
#ifdef KANJI
    return(ldelete(chunk, TRUE, FALSE));
#else
...
/*-----------------------------------------------------------------------------------
 * setmode -- prompt and set an editor mode
 *-----------------------------------------------------------------------------------
 */
_setmode(f, n)  
...
/*------------------------------------------------------------------------------------
 * setgmode -- prompt and set a global editor mode
 *------------------------------------------------------------------------------------
 */
setgmode(f, n)  
int f, n;           /* default and argument */
...
/*-----------------------------------------------------------------------------------
 * delgmode -- prompt and delete a global editor mode 
 *-----------------------------------------------------------------------------------
 */
delgmode(f, n)  
int f, n;           /* default and argument */
...
/*---------------------------------------------------------------------------------------
 * clrmes --    This function simply clears the message line,mainly for macro usage     
 *---------------------------------------------------------------------------------------
 */
clrmes(f, n)
...
/*----------------------------------------------------------------------------------------
 * writemes -- This function writes a string on the message line mainly for macro usage
 *----------------------------------------------------------------------------------------
 */
writemsg(f, n)
int f, n;                           /* arguments ignored        */
...
/*----------------------------------------------------------------------------------------
 * fmatch -- Close fences are matched against their partners, and if
...
/*--------------------------------------------------------------------------------------
 * istring -- ask for and insert a string into the current
