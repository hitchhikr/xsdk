...
register int c;     /* sequence to translate */
char *seq;          /* destination string for sequence */
{
    register char *ptr; /* pointer into current position in sequence */
...
/*----------------------------------------------------------------
 * help -- give me some help!!!! bring up a fake buffer and read
 * the help file into it with view mode         
 *----------------------------------------------------------------
 */
help(f, n)
{
    ...
    if( bp == NULL )
    {
        /* search through the list of help files( root -> bin -> emacs )*/
        for( i=2; i<NPNAMES; i++ )
        {
    ...
    return( TRUE ); /* OK Help File Setting */
}
...
unsigned char   *sfname;        /* name of startup file (null if default)   */
{
    register int status;            /* status of I/O operations             */
    register int i;                 /* index into help file names           */
    unsigned char fname[NSTRING];   /* buffer to construct file name in     */
    unsigned char *sfroot;          /* root of startup file name            */
    unsigned char *homedir;         /* pointer to your home directory       */

    if( sfname[0] == 0 )
    {
        sfroot = (unsigned char *)pathname[0];  /* default file name pointer */
    }
    else
    {
        sfroot = sfname;    /* call file name pointer */
    }
