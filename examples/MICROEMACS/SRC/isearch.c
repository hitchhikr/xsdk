...
int risearch(f, n)
{
    ...
    if (!(ists = isearch(f, -n)))   /* Call ISearch backwards         */
    {
        /* If error in search:            */
...
/*  } else mlerase ();  */          /* If happy, just erase the cmd line  */
    ...
    {
        if (ists == 1)              /* If happy, just erase the cmd line  */
        {
            mlerase ();
        }
    }
...
int fisearch(f, n)
{
    ...
    if (!(ists = isearch(f, n)))    /* Call ISearch forwards          */
    {
        /* If error in search:            */
        curwp->w_dotp = curline;    /* Reset the line pointer         */
        curwp->w_doto = curoff;     /*  and the offset to original value  */
...
#ifdef  KANJI
    int k_flag;         /* Kanji first byte flag          */
#endif
...
start_over:
...
#ifdef  KANJI
                if (ismsk2(cmd_buff,cmd_offset-1))
                {
                    --cmd_offset;
                }
#endif
    ...
    if (!k_flag && mskanjip(c))
    {
        ...
        {
            c = get_char ();        /* dummy read the lower byte  */
            cpos++;
        }
    }
    else
    ...
    if (!status)
    {
        /* If we lost last time       */
#ifdef  KANJI
        if (!k_flag)
        {
#endif
            Term_beep();            /* Feep again             */
#ifdef  KANJI
        }
#endif
    }
...
int sts;                /* Search status             */
{
    ...
    if (!sts)
    {
        return(FALSE);        /* Don't try unless ok so far         */
    }
...
#ifdef  KANJI
        if (!status && chklmsk(curline, curoff - 1))
        {
            curwp->w_doto--;
        }
#endif
...
int sts;                        /* previous search status             */
{
    ...
    if (!sts)
    {
        Term_beep();                /* Feep if search fails       */
    }
...
int col;    /* column to be echoed in */
{
    ...
    if ((c < ' ') || (c == 0x7F))               /* Control character?         */
    {
        switch (c)                              /* Yes, dispatch special cases*/
        {
            ...
            case 0x7F:                          /* Rubout:                  */
                Term_putc('^');                 /* Output a funny looking   */
                Term_putc('?');                 /* indication of Rubout     */
                col++;                          /* Count the extra char     */
                break;

            default:                            /* Vanilla control char     */
                Term_putc('^');                 /* Yes, output prefix       */
                Term_putc(c+0x40);              /* Make it "^X"             */
                col++;                          /* Count this char          */
        }
    }
    else
    {
        Term_putc(c);           /* Otherwise, output raw char */
    }
