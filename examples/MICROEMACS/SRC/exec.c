...
int f, n;       /* default Flag and Numeric argument */
{
    ...
    unsigned char cmdstr[NSTRING];  /* string holding command to execute */
 ...
 *--------------------------------------------------------------------------- */
docmd(cline)
unsigned char *cline;       /* command line to execute  */
{
...
/*--------------------------------------------------------------------------
 * gettok:  chop a token off a string return a pointer past the token
 *-------------------------------------------------------------------------- */
unsigned char *gettok(src, tok)
unsigned char *src, *tok;   /* source string, destination token string  */
{
...
/*--------------------------------------------------------------------------
 * nxtarg -- grab the next token out of sarg, return it, and chop it of sarg
 *--------------------------------------------------------------------------  */
nxtarg(tok)
unsigned char *tok;     /* buffer to put token into */
{
    register unsigned char *newsarg;/* pointer to new begining of sarg */
    register oldexec;               /* saved execution flag */
    register BUFFER *bp;            /* ptr to buffer to get arg from */
    register int status;
    unsigned char *gettok();

    newsarg = gettok(sarg, tok);    /* grab the token */
    strcpy(sarg, newsarg);          /* and chop it of sarg */
    /* check for an interactive argument */
    if (*tok == '@')
    {   /* get interactive argument */
...
int n;                  /* macro number to use      */
{
    ...
    unsigned char bname[NBUFN]; /* name of buffer to use    */
...
int f, n;                       /* default flag and numeric arg */
{
    ...
    register int status;        /* status return        */
    unsigned char bufn[NBUFN];  /* name of buffer to execute    */
...
BUFFER *bp;         /* buffer to execute */
{
    ...
    unsigned char eline[NSTRING];   /* text of line to execute  */
    ...
    while (lp != hlp)
    {
        ...
        {
            status = docmd(eline);  /* command execute  */
            if( status != TRUE )
            {   /* a command error  */
        ...
        lp = lp->l_fp;  /* skip on to the next line */
    }
    return(TRUE);
}

/*--------------------------------------------------------------------------
 * exexfile --  execute a series of commands in a file
 *-------------------------------------------------------------------------- */
execfile(f, n)
int f, n;           /* default flag and numeric arg to pass on to file  */
{
    register int status;            /* return status of name query      */
    unsigned char *fname[NSTRING];  /* name of file to execute      */
 ...
 *---------------------------------------------------------------------------- */
dofile(fname)
unsigned char *fname;           /* file name to execute */
{
    ...
    unsigned char bname[NBUFN]; /* name of buffer               */
