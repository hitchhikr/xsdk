...
linsert(n, c)
{
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        if (mskanjip(c >> 8))
        {
            lp2->l_text[doto+i] = (c >> 8) & 0xff;
            i++;
            lp2->l_text[doto+i] = c & 0xff;
        }
        else
#endif
        {
 ...
 */
ldelete(n, kflag, uflag)
long n;     /* # of chars to delete */
int kflag;  /* put killed text in kill buffer flag */
int uflag;  /* if TRUE, 1st arg "n" is not possibly equal # of bytes */
{
    register char *cp1;
    register char *cp2;
    register LINE *dotp;
...
#ifdef KANJI
    /* if top of YANK buffer is Shift-JIS code, redisplay current line */
    if (mskanjip(*(kbufh->d_chunk)))
    {
        ipcsts = 1;         /* see edef.h */
    }
#endif
