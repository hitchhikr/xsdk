...
register BUFFER *bp;
{
    ...
    if (--curbp->b_nwnd == 0)
    {
        ...
        up->b_dotp = wp->w_dotp;
        up->b_doto = wp->w_doto;
        up->b_markp = wp->w_markp;
        up->b_marko = wp->w_marko;
    }
    /* Switch. */
    curbp = bp; 
    ...
    wp=curwp;
    wp->w_bufp = bp;
    wp->w_linep = bp->b_linep;                  /* For macros, ignored. */
...
killbuf2(f,n)   /* original killbuffer */
{
    register BUFFER *bp;
    register int s;
    char bufn[NBUFN];
